...
def parse_java_result(path, n_threads, extra_fields=None):
    """
    ...
    extra_fields: list of top-level JSON field names to also carry through.
    """
    with open(path) as f:
        raw = json.load(f)
    ...
    record = {
        ...
        # well-known combining metrics (0.0 if absent — DS didn't report them)
    ...
    for field in (extra_fields or []):
        if field not in record:
            record[field] = r.get(field, 0.0)

    return record


def load_ds_dir(ds_dir, extra_fields=None):
    records = []
    for p in sorted(ds_dir.glob("result_cops*.json")):
        ...
        rec = parse_java_result(p, n_threads, extra_fields)
        records.append(rec)
...
def discover_targets(results_dir, requested, repeat_names, agg, extra_fields=None):
...
def fig_width(ticks, base=9.0, per_tick=0.55, minimum=7.0):
    """Return a figure width that keeps tick labels from overlapping."""
...
def draw_core_vline(ax, num_cores):
    """Draw a vertical reference line at num_cores with a label."""
    if num_cores is None:
        return
    ax.axvline(x=num_cores, color="#888888", linewidth=1.2,
               linestyle="dotted", zorder=1)
    # place label just inside the top of the axes
    ylim = ax.get_ylim()
...
def plot_throughput(targets, output_dir, agg, multi, thread_type, num_cores=None):
    ticks = all_thread_ticks(targets)
    fig, ax = plt.subplots(figsize=(fig_width(ticks), 5))
...
def plot_ops_breakdown(targets, output_dir, agg, multi, num_cores=None):
    ticks = all_thread_ticks(targets)
    w = fig_width(ticks, base=7.0) * 2
    fig, axes = plt.subplots(1, 2, figsize=(w, 5))
    ax_add, ax_rem = axes
    for i, (ds, records) in enumerate(targets.items()):
        x     = [r["threads"] for r in records]
...
def plot_metric(targets, output_dir, field, agg, multi,
                ylabel=None, title=None, num_cores=None):
    """
    Generic plot for any scalar field present in the aggregated records.
    Skips silently if no DS has non-zero values for the field.
    """
    # check that at least one DS has data for this field
    has_data = any(
        any(r.get(field, 0) != 0 for r in recs)
        for recs in targets.values()
    )
    if not has_data:
        print("  skip: no data for field '{}'".format(field))
        return
    ...
    setup_xaxis(ax, ticks)
    ax.set_ylabel(ylabel or field, fontsize=10)
    agg_note = " [{}{}]".format(
        agg, " " + band_label(agg) if band_label(agg) else "") if multi else ""
    ax.set_title((title or field) + agg_note, fontsize=11, fontweight="bold")
    ax.legend(framealpha=0.4, fontsize=9)
    draw_core_vline(ax, num_cores)
    style_ax(ax)
    fig.tight_layout()

    safe_name = re.sub(r"[^A-Za-z0-9_\-]", "_", field)
    out = output_dir / "java_{}_vs_threads.png".format(safe_name)
    fig.savefig(out)
    plt.close(fig)
    print("  saved:", out)
...
Examples:
  ...
  # built-in combining metrics + custom extra field
  python3 plot_java_sweep.py --results-dir sweep_results \\
    --stat throughput ops_per_combine nanos_per_combine \\
    --extra-metrics combineRounds avgQueueLength
        """
    )
    ...
    parser.add_argument("--thread-type", default="unknown",
                        choices=["unknown", "os", "virtual"])
    parser.add_argument("--num-cores", type=int, default=None,
                        metavar="N",
                        help="Draw a vertical reference line at this thread count ")
    ...
    all_extra = list(dict.fromkeys(
        ["opsPerCombine", "nanosPerCombine"] + args.extra_metrics
    ))
    ...
    if "throughput" in stat_set:
        plot_throughput(targets, output_dir, args.agg, multi, args.thread_type,
                        num_cores=args.num_cores)
    if "ops_per_combine" in stat_set:
        plot_metric(targets, output_dir,
                    field="opsPerCombine", agg=args.agg, multi=multi,
                    ylabel="ops per combining round",
                    title="ops per combine vs threads",
                    num_cores=args.num_cores)
    if "nanos_per_combine" in stat_set:
        plot_metric(targets, output_dir,
                    field="nanosPerCombine", agg=args.agg, multi=multi,
                    ylabel="ns per combining round",
                    title="nanos per combine vs threads",
                    num_cores=args.num_cores)
    if "attempts_per_combine" in stat_set:
        plot_metric(targets, output_dir,
                    field="attemptsPerCombine", agg=args.agg, multi=multi,
                    ylabel="attempts per combining round",
                    title="attempts per combine vs threads",
                    num_cores=args.num_cores)
    if "ops_breakdown" in stat_set:
        plot_ops_breakdown(targets, output_dir, args.agg, multi,
                           num_cores=args.num_cores)
    if "per_thread" in stat_set:
        plot_per_thread_boxplot(targets, output_dir)
    if "summary" in stat_set:
        print_summary(targets, output_dir, args.agg, multi)
