...
public class FcStackOptMatrix implements CompositionalQueue<Integer> {
    private static final int PARTS = 16;
    private static final int PART_LIMIT = 128;
    private static final int FC_ATTEMPTS = 3;
    ...
    public FcStackOptMatrix() {
        this.lock = new TTASLock();
        this.fcRequestSlots = new FcRequest[PARTS][PART_LIMIT];
        this.stack = new ArrayDeque<>(1_000_000);

        for (int i = 0; i < PARTS; i++) {
            fcRequestSlots[i] = new FcRequest[PART_LIMIT];
            for (int j = 0; j < PART_LIMIT; j++) {
                fcRequestSlots[i][j] = new FcRequest();
            }
        }
    }
    ...
    private FcRequest getRequest() {
        var tid = GlobalScopedValues.THREAD_ID.get();
        var part = tid / PART_LIMIT;
        var idx = tid % PART_LIMIT;
        return fcRequestSlots[part][idx];
    }
    ...
    void combine() {
        ...
        var part = GlobalScopedValues.THREAD_ID.get() / PART_LIMIT;
        for (int t = 0; t < FC_ATTEMPTS; ++t) {
            int ops = 0;
            for (int i = 0; i < registeredThreadsCount; i++) {
                var req = fcRequestSlots[part][i];
